...
int edu_dma_transfers(struct edu_dev *ed, dma_addr_t dma_addr,  int dir, size_t size)
{
	int ret = 0;
	u32 src, dst;
	...
	iowrite32(EDU_DMA_CMD_BUILD(dir, ed->interrupts_enabled),
		ed->bar0 + EDU_BAR0_DMA_CMD_REG);

	if (!ed->interrupts_enabled)
	{
		// poll for ocmption (bit 0 clears) with a timeout
		unsigned long timeout = jiffies + msecs_to_jiffies(ed->ms_wait_for_dma);
	...
	{
		if (!wait_for_completion_timeout(&ed->done, msecs_to_jiffies(ed->ms_wait_for_dma)))
		{
			ret = -ETIMEDOUT;
		}
	}
...
ssize_t edu_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	struct edu_dev *dev = filp->private_data;
	int ret;
...
struct pci_error_handlers edu_err_handlers = {
